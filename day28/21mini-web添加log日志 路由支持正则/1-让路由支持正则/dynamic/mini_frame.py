...
def index():
    ...
    # 创建Connection连接
    conn = connect(host='192.168.19.130', port=3306, user='root', password='123', database='stock_db', charset='utf8')
    # 获得Cursor对象
    cs = conn.cursor()
...
def center():
    ...
    # content = re.sub(r"\{%content%\}", my_stock_info, content)
    # 创建Connection连接
    conn = connect(host='192.168.85.128', port=3306, user='root', password='123', database='stock_db', charset='utf8')
    # 获得Cursor对象
    cs = conn.cursor()
    cs.execute(
