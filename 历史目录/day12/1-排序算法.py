...
class MySort:
    ...
    def small_heap_adjust(self, pos, len):
        """
        调整为小根堆
        :param pos:
        :param len:
    ...
    def heap_sort_small(self):
        ...
        for i in range(self.n // 2 - 1, -1, -1):
            self.small_heap_adjust(i, self.n)  # 建立小根堆,堆顶为最小元素
        arr[self.n - 1], arr[0] = arr[0], arr[self.n - 1]
        for i in range(self.n - 1, 1, -1):
            self.small_heap_adjust(0, i)
    ...
    def test_sort(self, sort_method, *args, **kwargs):
        ...
        end_time=time.time()
        print(f'程序耗时{end_time-start_time}s')
        #print(self.arr)
...
if __name__ == '__main__':
    ...
    # s.test_sort(s.heap_sort_big)
    s.test_sort(s.count_sort)