...
class PlaneGame:
    ...
    def __check_collide(self):  # 碰撞检测
        ...
        pygame.sprite.groupcollide(self.hero.bullets, self.enemy2_group, True, False)
        pygame.sprite.groupcollide(self.hero.bullets2, self.enemy_group, True, True)
        pygame.sprite.groupcollide(self.hero.bullets2, self.enemy2_group, True, True)


        # 2. 敌机撞毁英雄
        enemies = pygame.sprite.spritecollide(self.hero, self.enemy_group, True)
        enemies2 = pygame.sprite.spritecollide(self.hero, self.enemy2_group, True)

        # 判断列表时候有内容
        if len(enemies) or len(enemies2) > 0:
            # 让英雄牺牲
            self.hero.kill()
    ...
    def __update_sprites(self):  # 更新各种图像的位置
